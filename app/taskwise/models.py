...
class Task:
    ...
    def __init__(self, task_id: str, task: str,
        ...
        self.date_completed = (date_completed if date_completed is None
                               else date.today())
...
class Model:
    ...
    def __init__(self) -> None:
        try:
            ...
            table_exists = self.cursor.execute("SELECT name FROM sqlite_master"
                                               + " WHERE type='table' "
                                               + "AND name='tasks'").fetchone()

            if not table_exists:
                self.cursor.execute("CREATE TABLE IF NOT EXISTS tasks(task_id"
    ...
    def insert_task(self, task: Task):
        ...
        with self.conn:
            self.cursor.execute("INSERT INTO tasks VALUES (?, ?, ?, ?, ?, ?)",
                                (task.task_id, task.task, task.category,
                                 task.status, task.date_added,
                                 task.date_completed))
    ...
    def update_task(self, task_id: str, task: str = None,
        ...
        if task is not None:
            with self.conn:
                self.cursor.execute("UPDATE tasks SET task=? where task_id=?",
                                    [task, task_id])
        if category is not None:
            with self.conn:
                self.cursor.execute("UPDATE tasks SET category=? where"
                                    + " task_id=?", [category, task_id])
        if status != "open":
            with self.conn:
                self.cursor.execute(
                    "UPDATE tasks SET status=?, date_completed=? where"
                    + " task_id=?",
